 ...
 * {@code __heap_base} global so {@link AnimationInstance} can start.
 */
final class RuntimeWasm {
    ...
    static Buf section(int id, Buf body) {
        byte[] b = body.toBytes();
        return new Buf().u8(id).uleb(b.length).bytes(b);
    }

    static byte[] module(Buf... sections) {
    ...
    static ExecutionContext memoryContext(int pages) {
    ...
    static byte[] infiniteLoopModule() {
        Buf types = new Buf().vec(1).raw(0x60, 0x00, 0x01, 0x7F); // ()->(i32)
     ...
     */
    static byte[] forkDeterminismModule() {
